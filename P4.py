...
def min_max(bin_tree):
  ...
  def subtree_min_max(subtree_root):
    if (subtree_root.left is None) and (subtree_root.right is None):
      ...
      if (subtree_root.left is None) and (subtree_root.right is not None):
        min_val,max_val=subtree_min_max(subtree_root.right)
        maxval=max(max_val,subtree_root.data)
        minval=min(min_val,subtree_root.data)
        return (minval,maxval)

      if (subtree_root.left is not None) and (subtree_root.right is None):
        min_val,max_val=subtree_min_max(subtree_root.left)
        maxval=max(max_val,subtree_root.data)
        minval=min(min_val,subtree_root.data)
        return (minval,maxval)

      if (subtree_root.left is not None) and (subtree_root.right is not None):
        min_val_right,max_val_right=subtree_min_max(subtree_root.right)
        min_val_left,max_val_left=subtree_min_max(subtree_root.left)
        maxval=max(max_val_right,max_val_left,subtree_root.data)
        minval=min(min_val_right,min_val_left,subtree_root.data)
        return (minval,maxval)
  ...
  if bin_tree.is_empty()==False:
    return subtree_min_max(bin_tree.root)

  else:
    raise Exception("Minimum and maximum values are not defined for an empty binary tree")
